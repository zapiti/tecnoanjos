...
class DashChat extends StatefulWidget {
  /// Flex value for the  container defaults to 1
  /// Made so that the message container takes as much as possible
  /// if no height or width is passed
  ...
  ///  that start from left like English &
  ...
  /// with the [ChatMessage]  before make calls.
  ...
  /// Should the avatar be shown for every message  to false.
  ...
  /// [onLongPressAvatar] function  a function with this structure
  /// [Function(ChatUser)] will trigger when the avatar
  /// is long pressed
  final Function(ChatUser) onLongPressAvatar;

  /// [onLongPressMessage] function a function with this structure
  ...
  /// Main input length of the input text box to no limit.
  final int maxInputLength;

  /// Used to parse text to make it text uses
  ...
  /// and can also add custom  using regex
  ...
  /// will pass [Reply] as a  to function.
  ...
  /// load as you scroll scheme  will call the [onLoadEarlier]
  ...
  /// Override the default behaviour of the  Widget
...
class DashChatState extends State<DashChat> {
  ...
  @override
  void initState() {
    scrollController = widget.scrollController ?? ScrollController(initialScrollOffset: 0);
