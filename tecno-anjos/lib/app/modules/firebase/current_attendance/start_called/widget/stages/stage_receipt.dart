          ...
          () {
            if (_start < 1) {
              currentBloc.patchConclude(context, widget.attendance, () {
                ...
                profileBloc.getUserInfo(isReload: true);
              });
              timer?.cancel();
            } else {
              _start = _start - 1;
    ...
    return Container(
        ...
        child: ViewAttendanceWidget(
          ...
          childCenter: CenterViewAttendance(
              subtitle: _receiptCard(context, widget.attendance, false, null)),
          childBottom: Container(
              ...
              child: ElevatedButton(
                ...
                child: Text(
                  StringFile.fechar +
                      (ActivityUtils.isCancelTecno(widget.attendance)
                          ? ""
                          : "($_start)"),
...
Widget _receiptCard(final BuildContext context, final Attendance attendance,
    final bool showMoreDetail, final DateTime _dateTime) {
  return Container(
...
class ReceiptCard extends StatelessWidget {
  ...
  ReceiptCard(this.attendance, this.showMoreDetail, this._dateTime);
...
Widget _contentCard(
  ...
  return StreamBuilder<CurrentUser>(
      stream: _bloc.getCurrentUserFutureValue(),
      builder: (context, snapshot) =>snapshot.data == null ? loadElements(context): Card(
          shape: RoundedRectangleBorder(
          ...
          child: Container(
              ...
              child: Column(
                children: [
                  ...
                  snapshot.data?.role == CurrentUser.FUNCIONARY || attendance.status == ActivityUtils.CANCELADO
                      ? SizedBox()
                      : Text(
                          StringFile.total,
                          style: AppThemeUtils.normalSize(
                  ...
                  snapshot.data?.role == CurrentUser.FUNCIONARY || attendance.status == ActivityUtils.CANCELADO
                      ...
                      : Container(
                          ...
                          child: Text(
                            (" ${ActivityUtils.getTotalWithListMoney(attendance)}"),
                            style: AppThemeUtils.normalBoldSize(
                                fontSize: 24,
                                color: AppThemeUtils.colorPrimary),
                          )),
    ...
    return Container(
        ...
        child: Column(
          ...
          children: [
                        ...
                        (attendance?.isFavorite ?? false)
                            ? Container(
                                ...
                                child: Column(
                                  ...
                                  children: <Widget>[
                                    Text(attendance?.userClient?.name ?? "",
                                        style: AppThemeUtils.normalBoldSize(
                                            fontSize: 20)),
                                    SizedBox(
                                      height: 5,
            ...
            !(showImage ?? false) ? SizedBox() : lineViewWidget(top: 10),
            Row(
              children: [
                Expanded(
                    child: titleDescriptionWebWidget(context,
...
_infoTecno(Attendance attendance) {
  return Row(mainAxisAlignment: MainAxisAlignment.start, children: [
    ((attendance?.userTecno?.avaliations ?? "0.0") == "0.0")
        ? SizedBox()
        : Container(
            height: 24,
...
Widget itensOnDetailsOnly(
  ...
  return Column(
    children: [
      ...
      attendance.hourAttendance == null ? Container() : lineViewWidget(top: 10),
    ],
  );
}
