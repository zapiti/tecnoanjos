...
import 'package:tecnoanjosclient/app/modules/general/home/modules/avaliation/models/total_avaliation.dart';
...
class AvaliationRepository {
  ...
  Future<ResponsePaginated> getListAvaliationMaker({int page}) async {
    ...
    var listhours = await _requestManager.requestWithTokenToForm(AVALIATION,Flavor.I.getString(Keys.apiUrl),
  ...
  Future<ResponsePaginated> getListAvaliationPendent({int page}) async {
    var elements = await _requestManager.requestWithTokenToForm(AVALIATION,Flavor.I.getString(Keys.apiUrl),
    ...
    if(elements.content != null){
      try{
        ...
        var tempList = list.where((element) => element.attendance.status == ActivityUtils.FINALIZADO && element.attendance.userTecno != null).toList();
